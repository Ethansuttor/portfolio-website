...
const DynamicPcbGlbCanvas = dynamic(() => import('@/components/PcbGlbCanvas'), {
  ssr: false,
  loading: () => (
    <div className="w-full h-[420px] bg-surface-container-high/60 border border-outline-variant/20 flex flex-col items-center justify-center gap-2 text-xs font-mono text-primary animate-pulse rounded-md">
      <span className="h-2 w-2 rounded-full bg-primary animate-ping" />
      <span>Loading the 3D board model…</span>
    </div>
  ),
});
